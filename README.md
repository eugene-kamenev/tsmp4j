...
![Matrix profile of a synthetic series](examples/matrix-profile-sample.png)

*Sample output: a synthetic series of two recurring motifs (a sine wave and a sawtooth, each repeated) plus
one unique waveform spliced into the middle. Its matrix profile (STOMP, window size 30) stays low wherever a
subsequence has a good match — the recurring motifs — and spikes at the discord, the subsequence that matches
nothing. The data is generated by [`examples/GenerateSample.java`](examples/GenerateSample.java) and plotted
by [`examples/plot.py`](examples/plot.py); see [Building and testing](#building-and-testing) to reproduce it.*
...
### Generating the sample figure

The figure at the top of this page is reproducible. `examples/GenerateSample.java` builds a deterministic
synthetic series and computes its matrix profile with STOMP, writing CSVs under `examples/data/`;
`examples/plot.py` renders those CSVs into `examples/matrix-profile-sample.png` (needs Python with
...
java  -cp "$CP:/tmp/examples-classes" GenerateSample examples/data   # writes examples/data/*.csv
python3 examples/plot.py                                             # writes examples/matrix-profile-sample.png
```

`GenerateSample` uses a fixed seed, so the series, the profile and the figure are byte-for-byte reproducible.
...
examples/
├── GenerateSample.java   generates the sample series + matrix profile CSVs
├── plot.py               renders examples/matrix-profile-sample.png from those CSVs
├── data/                 timeseries.csv, matrix_profile.csv
└── matrix-profile-sample.png
```
